...
def tarrant_cad(address):
    ...
    dsoup = BeautifulSoup(requests.get(detail_url, timeout=10).text, 'html.parser')
...
# --- 3 Owner profile & web search (unchanged stub + googlesearch) ---
...
def get_market_comps(lat, lng):
    ...
    return {'competitors_5': c5, 'count_5': cnt5, 'density_5': d5,
            'competitors_10': new10, 'count_10': cnt10, 'density_10': d10}


# --- 5 New: scrape comparables from Crexi & Loopnet (basic) ---
def scrape_crexi(lat, lng, radius_m=1):
    url = f"https://www.crexi.com/search/properties?property_type=Self+Storage&lat={lat}&lng={lng}&radius={radius_m}"
    html = requests.get(url, timeout=10).text
    soup = BeautifulSoup(html, 'html.parser')
    listings = []
...
# --- 6 New: tax history stub (replace with real scraper as needed) ---
def get_tax_history(address):
    # stubbed example data
    return [
        {'year': 2023, 'tax': 3200},
        {'year': 2022, 'tax': 3000},
        {'year': 2021, 'tax': 2800}
    ...
    if request.method == 'POST':
        ...
        if not addr_in and not fac_in:
            ...
            if geo.get('status') != 'OK':
                ...
                # deal metrics stub (you can replace nrsf with real value if you have it)
                ...
                if listings:
                    avg_ppsf = round(sum(l['ppsf'] for l in listings) / len(listings), 2)
                else:
                    avg_ppsf = 0
                rec_value = round(avg_ppsf * nrsf, 2)
